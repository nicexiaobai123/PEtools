...
int main()
{
	...
	PVOID cur_load_dll = (PVOID)LoadLibrary("壳.dll");
	...
	PVOID temp = (PVOID)((long)pet_exe.get_filebuffer() + shell_start_foa);	// 临时内存下PE结构,新添加的节处开始
	PEtools temp_pet(temp);
	if (!temp_pet.is_success())return false;

	long cur_imagebase = pet_exe.imagebase() + shell_start;
	...
	// --------------------------- 入口、tls处理 ---------------------------------
		// 设置oep为 壳的oep
	long dll_oep = Share->new_oep - (long)cur_load_dll;			// 壳oep,是rva
	...
	//	保存
	ret = pet_exe.to_file("D:\\123.exe");
	if (!ret) return false;

	//	卸载dll
	FreeLibrary((HMODULE)cur_load_dll);

	if (ret)cout << "加壳成功到文件" << endl;
	system("pause");
	return 0;
}